const EVENT_ID = process.env.ESPN_EVENT_ID || "401811963";
...
const EVENT_VENUE = process.env.EVENT_VENUE || "Bellerive Country Club";
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!event || !competition) throw new Error("Selected golf event was not found");
    const currentRound = competition.status?.period || 1;
    const par = Number.isFinite(EVENT_PAR) ? EVENT_PAR : 71;
